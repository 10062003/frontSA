...
const TablaProfesiones = () => {
  const servicioProfesiones = new ServiciosProfesion();
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  const ObtenerDatosTabla = async () => {
    const respuesta = await servicioProfesiones.ListarProfesiones();
    if (respuesta.respuesta === 1) {
      setData(respuesta.listaProfesiones);
    } else {
  ...
  const columns = [
    ...
    {
      header: "ID Estado",
      accessorKey: "mPfsIdEstado",
      hidden: true,
    },
    {
      header: "Estado",
      accessorKey: "mPfsEstado",
      cell: ({ cell }) => (
        <div
          style={{
            display: "flex",
...
export default TablaProfesiones;
